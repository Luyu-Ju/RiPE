...
# Moussaïd, M et al., 2010 The walking behaviour of pedestrian social groups and its impact on crowd dynamics
...
class Force(ABC):
    ...
    @abstractmethod
    def _get_force(self, stepn) -> np.ndarray:
        """Abstract class to get social forces
            return: an array of force vectors for each pedestrians
        """
        raise NotImplementedError

    def get_force(self, stepn, debug=False):
        force = self._get_force(stepn)
        if debug:
...
class DesiredForce(Force):
    ...
    def _get_force(self, stepn):
        ...
        # read target of last step
        targetLast = self.peds.target() 
        coorpath = self.peds.coorpath
        guidelineIndex = self.peds.guidelineIndex
        targetnewcoor = stateutils.target(pos, targetLast, coorpath, guidelineIndex)[1]
        
        elapsed_time = (stepn+1)*self.peds.step_width
        time_mask = self.peds.pretime <= elapsed_time
        indices = np.where(time_mask == False)
        
        direction, dist = stateutils.normalize(targetnewcoor - pos)
...
class SocialForce(Force):
    ...
    def _get_force(self, stepn):
        ...
        elapsed_time = (stepn+1)*self.peds.step_width
        time_mask = self.peds.pretime <= elapsed_time
        numped = np.count_nonzero(time_mask == True)
        indices = np.where(time_mask == True)
        
        posvec = self.peds.pos()
        posvec = posvec[time_mask]
        velvec = self.peds.vel()
        velvec = velvec[time_mask]
        pos_diff = stateutils.each_diff(posvec)  # n*(n-1)x2 other - self
        diff_direction, diff_length = stateutils.normalize(pos_diff)
        vel_diff = -1.0 * stateutils.each_diff(velvec)  # n*(n-1)x2 self - other
        ...
        forcePed = force_velocity + force_angle  # n*(n-1) x 2
        
        #
        # posevacuee = self.peds.state[:,0:2]
        posshelter = self.peds.goal()
        expanded_posvec= posvec[:, np.newaxis, :]
        expanded_posgoal = posshelter[np.newaxis, :, :]
        distances = np.linalg.norm(expanded_posvec - expanded_posgoal, axis=2)
        indicesStop = np.min(distances, axis=1) < 3
        
        indicesDist = diff_length > 5
        forcePed[indicesDist] = 0

        if numped > 0:
            forcePed = np.sum(forcePed.reshape((numped, -1, 2)), axis=1)  # self.peds.size()
        
        forcePed[indicesStop] = 0
        
        force = np.zeros((self.peds.size(), 2))
        force[indices[0]] = forcePed
        return force * 4.5
...
class ObstacleForce(Force):
    ...
    def _get_force(self, stepn):
        sigma = 0.1  # sigma = self.config("sigma", 0.2)  # change
        ...
        forcePed = np.zeros((self.peds.size(), 2))
        if len(self.scene.get_obstacles()) == 0:
            return forcePed
        
        # for i, p in enumerate(pos):
        #     diff = p - obstacles 
        diff = np.expand_dims(obstacles, axis=0) - pos[:, np.newaxis]
        directions, dist = stateutils.normalize3(diff)
        dist = dist - self.peds.agent_radius
        dist = dist[:, :, np.newaxis]
        # if np.all(dist >= threshold):
        #     continue
        dist3d = dist[:, :, np.newaxis]
        dist_mask3d = np.where(dist3d < threshold-3.05)
        # directions[dist_mask3d[0],dist_mask3d[1],:] *= np.exp(-dist3d[dist_mask3d].reshape(-1, 1) / sigma)
        directions[dist_mask3d[0],dist_mask3d[1],:] *= np.exp(-dist3d[dist_mask3d].reshape(-1, 1) / sigma)
        indices_axis0 = dist_mask3d[0]
        indices_axis1 = dist_mask3d[1]
        page_indices0 = np.split(indices_axis0, np.unique(dist_mask3d[0], return_index=True)[1][1:])
        page_indices1 = np.split(indices_axis1, np.unique(dist_mask3d[0], return_index=True)[1][1:])
        
        if len(page_indices0[0]) != 0:
            for i in range(len(page_indices0)):
                id = page_indices0[i][0]
                forcePed[id] = np.sum(directions[page_indices0[i][0],page_indices1[i],:], axis = 0)

        force = np.zeros((self.peds.size(), 2))
        force[indices[0]] = forcePed[indices[0]]
        # force[indices] = 0
        return force * 3
        # return force * 0.1
...
#             forcePed[i] = np.sum(directions[dist_mask], axis=0)

#         force = np.zeros((self.peds.size(), 2))
#         force[indices] = forcePed
#         return force * self.factor
